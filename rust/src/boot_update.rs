...
impl FileMetadata {
    pub(crate) fn new_from_path<P: openat::AsPath>(
        ...
        let digest = bs58::encode(hasher.finish().expect("completing hash")).into_string();
        Ok(FileMetadata {
            size: meta.len(),
            sha512: digest,
        })
    ...
    pub(crate) fn extend_hash(&self, hasher: &mut Hasher) {
        ...
        hasher.update(self.sha512.as_bytes()).unwrap();
    }
}
...
mod ffi {
    ...
    pub extern "C" fn ror_boot_update_entrypoint(
        ...
        int_glib_error(boot_update_main(&v), gerror)
    }
}
pub use self::ffi::*;
