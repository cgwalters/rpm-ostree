...
struct RpmFileDb final
{
  ...
  void insert_entry (std::string_view pkg_nevra, std::string_view pkg_path);

  rust::Vec<rust::String> find_pkgs_for_file (rust::Str path) const;
...
private:
  ...
  bool fs_paths_are_equivalent (cached_string dirname, const RpmFileDb::PkgFileInfo &entry) const;

  std::optional<cached_string> try_resolve_real_fs_path (cached_string path) const;
