...
RpmFileDb::insert_entry (std::string_view pkg_nevra, std::string_view pkg_path)
{
  ...
  basename_to_pkginfo[str_cache.get_or_insert (basename_v)].emplace_back (RpmFileDb::PkgFileInfo{
      str_cache.get_or_insert (pkg_nevra), str_cache.get_or_insert (dirname_v) });
}
...
bool
RpmFileDb::fs_paths_are_equivalent (cached_string dirname,
                                    const RpmFileDb::PkgFileInfo &entry) const
{
  ...
  while (!path_components.empty () && current_path != NULL)
    {
      ...
      g_autoptr (GFileInfo) childinfo
          = g_file_query_info (child, "*", G_FILE_QUERY_INFO_NONE, NULL, &error);
      if (childinfo == NULL)
        {
          g_autofree char *error
              = g_strdup_printf ("Failed to get file info for '%s'", g_file_peek_path (child));
      ...
      if (g_file_info_has_attribute (childinfo, "standard::is-symlink")
        ...
        {
          symlink_remapped_path = g_file_resolve_relative_path (
              current_path, g_file_info_get_symlink_target (childinfo));
        }

      // Update our tracked current path
      {
        GFile *temp = current_path;
        current_path = symlink_remapped_path != NULL ? G_FILE (g_object_ref (symlink_remapped_path))
          ...
          if (RPMFILE_IS_INSTALLED (rpmfiFState (fi)))
            {
              ...
              result->insert_entry (std::string_view (pkg_nevra.data (), pkg_nevra.size ()), path);
            }
        }
    }
